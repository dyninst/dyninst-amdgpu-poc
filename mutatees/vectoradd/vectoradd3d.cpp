// vectoradd3d — 3D-grid vectoradd mutatee (for the 2D/3D per-wave wid work).
//
// A 3D launch (grid/block use x,y,z) is the full exercise of the general wavefront-id
// path: the per-wave slice index must flatten all of blockIdx.{x,y,z},
// threadIdx.{x,y,z}, blockDim.{x,y,z}, gridDim.{x,y} consistently with the host's
// nwaves = numBlocks * ceil(blockDim/64), or slices collide. getpc-free.
//
...
__global__ void vectoradd3d(float* C, const float* A, const float* B, int W, int H, int D) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int z = blockIdx.z * blockDim.z + threadIdx.z;
    if (x < W && y < H && z < D) { int i = (z * H + y) * W + x; C[i] = A[i] + B[i]; }
}

int main() {
    HIP_CHECK(hipSetDevice(select_gfx908_device()));

    const int W = 16, H = 16, D = 8;               // 2048 elems; block8x8x4=4 waves, grid2x2x2 => 32 waves
    const int N = W * H * D;
    ...
    dim3 block(8, 8, 4);                           // 256 threads => 4 waves/block
    dim3 grid((W + block.x - 1) / block.x,
              (H + block.y - 1) / block.y,
              (D + block.z - 1) / block.z);
    hipLaunchKernelGGL(vectoradd3d, grid, block, 0, 0, dC, dA, dB, W, H, D);
