// vaddcall2d — 2D-grid vectoradd through a NON-INLINED __device__ call (getpc idiom).
//
// Combines the getpc-relocation mutatee (vaddcall.cpp) with a 2D launch (wgidX+wgidY
// enabled): validates that the getpc-call relocation AND the non-leaf scratch entry
// prologue work when the wavefront-offset SGPR lands after multiple work-group-id SGPRs.
// getpc idiom present (fadd not inlined); non-leaf (has a device call).
...
__global__ void vaddcall2d(float* C, const float* A, const float* B, int W, int H) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x < W && y < H) { int i = y * W + x; C[i] = fadd(A[i], B[i]); }  // getpc -> swappc (fadd)
}
...
int main() {
    ...
    hipLaunchKernelGGL(vaddcall2d, grid, block, 0, 0, dC, dA, dB, W, H);
